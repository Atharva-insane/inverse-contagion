...
def visualize_contagion(model_path='../models/nghp_model.pth', data_dir='../processed_data'):
    ...
    # Create an artificial severe delay at Airport 0 (e.g., ATL or ORD)
    seed_cascade = np.zeros((1, seq_len, num_nodes, node_features))
    seed_cascade[0, -1, 0, 0] = 300.0 # 300 min delay at the source
    ...
    att_from_source = last_step_alpha[:, 0] # How much airport 0 infected others
    ...
    plt.xlim(-125, -65)
    plt.ylim(25, 50)
    plt.title(f"Hawkes Process Contagion Spread from {top_airports[0]}", fontsize=16)
    ...
    source_lon = airport_meta.loc[top_airports[0], 'LONGITUDE']
    source_lat = airport_meta.loc[top_airports[0], 'LATITUDE']
    
    # Plot the source of disruption
    plt.scatter(source_lon, source_lat, c='red', s=200, marker='*', label=f'Source ({top_airports[0]})')
    ...
    out_path = '../output/hawkes_contagion_map.png'
